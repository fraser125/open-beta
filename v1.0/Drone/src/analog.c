...
void setupAnalog(void) {
	...
	// Configure the ADC
	adc_set_conversion_parameters(&analog_config, ADC_SIGN_OFF, ADC_RES_12, ADC_REF_AREFA); // Reference is VCC/1.6
	adc_set_conversion_trigger(&analog_config, ADC_TRIG_MANUAL, 1, 0);
	
	// Set the adc clock rate:
	adc_set_clock_rate(&analog_config, 200000UL);
...
uint16_t sampleAnalogChannel(enum adcch_positive_input channel) {
	...
	adc_wait_for_interrupt_flag(&JOYSTICK_ADC_MODULE, JOYSTICK_ADC_CHANNEL);

	// Return the result:
	return adc_get_result(&JOYSTICK_ADC_MODULE, JOYSTICK_ADC_CHANNEL);
};
